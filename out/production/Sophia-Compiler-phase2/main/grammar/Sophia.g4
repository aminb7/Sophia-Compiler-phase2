...
sophiaClass returns[ClassDeclaration classDec]: CLASS name = identifier (EXTENDS parent = identifier)?
	...
	LBRACE (((varDec1 = varDeclaration {if ($varDec1.text != null) $classDec.addField(new FieldDeclaration($varDec1.varDec));}
	| methodDec1 = method {if ($methodDec1.text != null) $classDec.addMethod($methodDec1.methodDec);}
    )* c = constructor {if ($c.text != null) $classDec.setConstructor($c.c);}
    (varDec2 = varDeclaration {if ($varDec2.text != null) $classDec.addField(new FieldDeclaration($varDec2.varDec));}
    | methodDec2 = method {if ($methodDec2.text != null) $classDec.addMethod($methodDec2.methodDec);}
    )*)
	| ((varDec = varDeclaration {if ($varDec.text != null) $classDec.addField(new FieldDeclaration($varDec.varDec));}
...
equalityExpression returns[Expression _eq]: e1 = relationalExpression {$_eq = $e1._rel;}
	((equal = EQUAL e2 = relationalExpression {$_eq = new BinaryExpression($_eq, $e2._rel, BinaryOperator.eq);})
	| (notEqual = NOT_EQUAL e2 = relationalExpression {$_eq = new BinaryExpression($_eq, $e2._rel, BinaryOperator.neq);})
	)*;

relationalExpression returns[Expression _rel]: e1 = additiveExpression {$_rel = $e1._add;}
	((gt = GREATER_THAN e2 = additiveExpression {$_rel = new BinaryExpression($_rel, $e2._add, BinaryOperator.gt);})
	| (ls = LESS_THAN e2 = additiveExpression {$_rel = new BinaryExpression($_rel, $e2._add, BinaryOperator.lt);})
	)*;

additiveExpression returns[Expression _add]: e1 = multiplicativeExpression {$_add = $e1._mult;}
 	((plu = PLUS e2 = multiplicativeExpression {$_add = new BinaryExpression($_add, $e2._mult, BinaryOperator.add);})
 	| (minu = MINUS e2 = multiplicativeExpression {$_add = new BinaryExpression($_add, $e2._mult, BinaryOperator.sub);})
 	)*;

multiplicativeExpression returns[Expression _mult]: e1 = preUnaryExpression {$_mult = $e1._preUnaryExpression;}
	((mul = MULT e2 = preUnaryExpression {$_mult = new BinaryExpression($_mult, $e2._preUnaryExpression, BinaryOperator.mult);})
	| (di = DIVIDE e2 = preUnaryExpression {$_mult = new BinaryExpression($_mult, $e2._preUnaryExpression, BinaryOperator.div);})
	| (mo = MOD e2 = preUnaryExpression {$_mult = new BinaryExpression($_mult, $e2._preUnaryExpression, BinaryOperator.mod);})
	)*;
